...
### Why did I do this
I thought that the touch sensor capabilities of the breadboard was really cool and I wanted to do something with it. When I learned that there was a speaker in the kit that could play different frequencies based on the code, I knew I wanted to build a little piano. This is a fun little project for anyone who is into computers or tech as well as sound because it lets you begin to understand how computers take in signals and return frequencies.
...
- An ESP32 Breadboard and Arduino IDE installed and running properly on your computer
...
If you want the buzzer to be quieter, treat it like it is an LED light, connecting the left pin to channel 21 and the right pin to ground, completely bypassing the transistor. The transistor allows enough power to be funneled into the Buzzer for it to play at full volume.

### Challenges and Interests in C
The most interesting part of this project (and the most frustrating) was that each channel required different touch threshholds which I had to find manually. However, after finding each individual threshhold I was able to pick a number that satisfied all of the threshholds to simplify the code. I also had to create a unique boolean condition for each channel so that they wouldn't cancel each other out. If there was just one condition check for all 8 jumpers then the sensors that weren't being touched would be constantly returning false causing the note to stutter and the Serial Monitor to be printing the note every tick because the computer is reading that the buzzer is being turned on and off at the same time constantly. By having a separate condition for each jumper it allows for a single, unbroken note to be played

### My Code
To install the code yourself go to 
  [MyCode](https://github.com/milburnk-beep/KVM210FinalPiano.io/blob/main/ArduinoPiano.C)
